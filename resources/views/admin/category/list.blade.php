    ...
    <div class="inside">
        <div class="contentbox-title">
            ...
            @if (!Request::is('admin/category'))
                <a href="{{ route('admin.category.index') }}" class="btn btn-primary">{{ __('static.categories.add_category') }}</a>
            @endif
        </div>
                        ...
                        <ol class="dd-list">
                            @if (isset($categories))
                                @forelse ($categories as $category)
                                    <li class="dd-item dd3-item {{ isset($cat) && $cat->id == $category->id ? 'active' : '' }}" data-id="{{ $category->id }}">
                                            ...
                                            <form method="POST" action="{{ route('admin.category.destroy', $category->id) }}">
                                                ...
                                                @php
                                                    $route =
                                                        ...
                                                        app()->getLocale();
                                                @endphp
                                                <a href="{{ $route }}" class="edit"><i class="ri-edit-2-line"></i></a>
                                            </form>
                                        </div>
