        ...
        <div class="wallet-detail">
            <div class="wallet-detail-content">
                    ...
                    <div class="form-group row">
                        @if (isset($label))
                            <label class="col-md-2"
                                for="name">{{ __('taxido::static.wallets.balance') }}<span>*</span></label>
                        @endif
                        <div class="col-md-10">
                            <h4>{{ getDefaultCurrency()?->symbol }}
                                <input class="form-control" type="text" name="name" min="1"
                                    id="balanceLabel" readonly
                                    value="{{ isset($balance) ? number_format($balance, 2) : '0.00' }}">
                            </h4>
                        </div>
                    </div>
                </div>
                ...
                @if (Auth::user()->hasRole(RoleEnum::ADMIN))
                    <form action="{{ route('admin.rider-wallet.update.balance') }}" method="POST" id=""
                        class="wallet-form">
                        @csrf
                        <input class="form-control riderId" type="hidden" name="rider_id"
                            value="{{ request()->query('rider_id') }}" placeholder="+ ">
    ...
    <script>
        (function($) {
            ...
            const balanceInput = () => {
                ...
                let isConsumerSelected = $("#select-rider").val();
                let disableButtons = (
                    balanceInput <= 0 ||
            ...
            $("#select-rider").on("change", balanceInput);
        })(jQuery);
    </script>
@endpush
